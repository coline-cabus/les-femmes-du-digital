    ...
    class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
            ...
            if ( !empty( $item->description ) ) {
                // Append an <div> with the content description.
                $description_html = '<div class="description">' . $item->description . '</div>';
            }

            $img_html = '';
            if ( $img = get_field('image', $item) ) {
                // Append an <img> with the content Image by ACF.
                $img_html = wp_get_attachment_image($img['id'], 'small', false, ['class' => 'menu-img']);
            }

            /** This filter is documented in wp-includes/post-template.php */
            $title = '<div class="title-menu">' . apply_filters( 'the_title', $item->title, $item->ID ) . '</div>';
             ...
             * @param int              $depth Depth of menu item. Used for padding.
             */
            $title = '<div class="title-menu">' . apply_filters( 'nav_menu_item_title', $title, $item, $args, $depth ) . '</div>';
            ...
            $item_output .= isset( $args->link_before ) ? $args->link_before . '<div class="img">' . $img_html . '</div>' . $icon_html . '<div class="dropdown-meta-container">' . $title . $description_html . '</div>' . $args->link_after : '';
            /*
